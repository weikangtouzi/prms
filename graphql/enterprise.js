...
const { Identity, EnterpriseNature } = require('./types/')
...
const editEnterpriseBasicInfo = async (parent, args, context, info) => {
  ...
  if (context.req && context.req.headers.authorization) {
    ...
    if (Identity.parseValue(userInfo.identity) == Identity.getValue("EnterpriseUser")) {
      
    } else {
      throw new AuthenticationError("only enterprise user could using enterprise functions")
    }
    // Enterprise.create({
